...
- FASE (detección-primero para recall · análisis diferido, opt-in `vitas_recall`): en 4K a plano completo la POSE detecta ~0 jugadores; el modelo de DETECCIÓN (`yolo11s-detect`, clase 0=person, el mismo del balón) con tiling recupera el EQUIPO COMPLETO (la caja tiene menor suelo de tamaño que la pose). El worker corre el detector con tiling → posición de todos; y la POSE solo sobre las cajas grandes (cercanas) → biomecánica solo donde hay píxeles fiables; los lejanos = posición sí, keypoints NO (nunca inventados). Módulos: `detectPostprocess.ts` (decodifica el detector, reutiliza el `globalNms` de tiling), `poseEligibility.ts` (frontera cercano/lejano + `cobertura_pose` como MetricResult), `recallPipeline.ts` (orquestador PURO), `recallConfig.ts` (opt-in). Coordina con `vitas_tiling`. El frame se extrae a 640² (igual que el vivo) para NO tocar la homografía; el techo del benchmark (6×6 @4K) exige extracción de alta resolución con consistencia de coordenadas (follow-up). PRECISIÓN real (recall/FP) exige ground truth ANOTADO A MANO (V6/`fixtures/`) — no dar números al usuario antes; el umbral de cercanía queda "pendiente de validar". Ver `docs/roadmap-vision-tracking.md`.
