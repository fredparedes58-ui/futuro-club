...
-- RLS: org members can read/write injuries for their org's players
CREATE POLICY "org_members_read_injuries" ON player_injuries
  FOR SELECT USING (
    org_id IN (SELECT org_id FROM team_members WHERE member_id = auth.uid())
...
CREATE POLICY "org_members_update_injuries" ON player_injuries
  FOR UPDATE USING (
    created_by = auth.uid()
    OR org_id IN (SELECT org_id FROM team_members WHERE member_id = auth.uid() AND role IN ('director', 'coach'))
  );
...
CREATE POLICY "snapshots_read_own" ON player_metric_snapshots
  FOR SELECT USING (
    player_id IN (
      SELECT id FROM players WHERE tenant_id = auth.uid()
      UNION
      SELECT p.id FROM players p
      JOIN team_members tm ON p.tenant_id = tm.org_id
