      ...
      <DrawerContent className="max-h-[85vh]">
        <DrawerHeader className="border-b border-border pb-4">
          ...
          <DrawerDescription className="text-muted-foreground">
            {t("evidenceDrawer.evidenceDetail")} — <span className="font-mono">{indicator.indicator}</span>
          </DrawerDescription>
        </DrawerHeader>
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <MetricBox label={t("evidenceDrawer.rawValue")} value={String(indicator.raw_value)} />
            <MetricBox label={t("evidenceDrawer.normalizedScore")} value={String(indicator.normalized)} color={indicator.normalized >= 70 ? "text-primary" : "text-electric"} />
            <MetricBox label={t("evidenceDrawer.reliability")} value={`${Math.round(indicator.reliability * 100)}%`} color={getConfidenceColor(indicator.reliability)} />
            <MetricBox label={t("evidenceDrawer.contribution")} value={`${indicator.contribution > 0 ? "+" : ""}${indicator.contribution.toFixed(2)}`} color={impactColor} />
          </div>
          ...
          <div className="flex gap-3">
            ...
            <Badge variant="outline" className={impactColor}>
              {t("evidenceDrawer.impact")} {indicator.impact}
            </Badge>
          </div>
          ...
          {/* Positions impacted */}
          <div>
            <h4 className="text-sm font-display font-semibold mb-2">{t("evidenceDrawer.positionsImpacted")}</h4>
          ...
          {/* Archetypes impacted */}
          <div>
            <h4 className="text-sm font-display font-semibold mb-2">{t("evidenceDrawer.archetypesImpacted")}</h4>
            <div className="flex flex-wrap gap-2">
