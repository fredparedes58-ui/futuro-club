...
import { engagementSnapshotToRow, rowToEngagementSnapshot } from "./engagementRow";
  ...
  async getEngagement(playerId: string, limit = 30): Promise<EngagementSnapshot[]> {
    if (SUPABASE_CONFIGURED) {
      try {
        ...
        if (error) throw error;
        if (data) {
          return (data as Array<Record<string, unknown>>).map(rowToEngagementSnapshot);
        }
      } catch (err) {
  ...
  async saveEngagement(snapshot: EngagementSnapshot): Promise<EngagementSnapshot> {
    ...
    if (SUPABASE_CONFIGURED) {
      try {
        await supabase
          .from("engagement_snapshots")
          .upsert(engagementSnapshotToRow(final), { onConflict: "id" });
      } catch (err) {
